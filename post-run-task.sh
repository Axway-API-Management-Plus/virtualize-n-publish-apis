...
do
    ...
    # Remove secrity elements to simplify usage of the mocked APIs.
    # FYI, the original files stay as-is, since we create modified spec files in a separate dir

    echo $(cat $FILE_NAME | jq 'del(.. | .security?)') > processed/$FILE_NAME
